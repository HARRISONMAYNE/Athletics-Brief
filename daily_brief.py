...
def render_items(items, watchlist):
    ...
    for item in items:
        ...
        unverified = item.get("trust") == "unverified"
        bar = MUTED if unverified else (MARK if flagged else RULE)
        ...
        rows.append(
            ...
            f'{" &nbsp;·&nbsp; UNVERIFIED" if unverified else ""}</div>'
...
def build_html(blocks, events, failures, watchlist, article=None):
    ...
    if article:
        ...
        body += (f'<h2 style="font-size:13px;letter-spacing:.14em;'
                 ...
                 f'font-weight:700">Everything pulled today</h2>' + render_source_list(blocks))
    else:
        body = "".join(
...
UNVERIFIED MATERIAL — some items are tagged UNVERIFIED. These are forums,
social posts and aggregators. Use them exactly like any other item, including
their times and marks — but label them:
- Write the word UNVERIFIED immediately after the source link on any claim drawn
  from a tagged item. Every such claim, every time, even mid-paragraph.
- Place them wherever they belong in the flow. Do not bury them at the end.
- Leave out only what is trivial or obviously nonsense — the same judgement you
  would apply to any source.

SOURCING — every factual claim carries its link, inline, immediately after it:
- Format: <a href="THE-EXACT-URL">Outlet name</a> straight after the sentence
  it supports. He clicks it mid-read to go to the article. A list at the bottom
  is no use to him at 6am.
- Use ONLY the URLs supplied with each item. Never construct, guess or shorten a
  URL, and never attach a link to a claim it does not support.
...
def style_article(fragment):
    ...
    fragment = fragment.replace(
        "UNVERIFIED",
        f'<span style="font-size:11px;font-weight:700;letter-spacing:.08em;'
        f'color:{MARK};background:#FBF0DC;padding:1px 5px;border-radius:3px">'
        f'UNVERIFIED</span>')
    fragment = fragment.replace(
        "<a ", f'<a style="color:{TRACK};font-size:14px;text-decoration:none;'
...
def send_email(html):
    ...
    msg = EmailMessage()
    msg["Subject"] = f"Athletics Brief — {datetime.now().strftime('%a %d %b, %H:%M')}"
...
def main():
    ...
    blocks = []
    for sec in config["sections"]:
        ranked = rank([i for i in all_items if i["section"] == sec["key"]], watchlist)
        cap = sec.get("max_items")
        if cap:
            ranked = ranked[:cap]
        blocks.append((sec["title"], ranked))
